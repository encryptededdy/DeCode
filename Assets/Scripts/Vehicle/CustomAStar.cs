...
namespace Vehicle
{
    public class CustomAStar
    {
        ...
        {
            get { return (evaluate, target) => Vector3.Distance(evaluate.Position, target.Position); }
        }
        ...
        public CustomAStar(Heuristic heuristic)
        {
        ...
        public IEnumerator SearchPath(INode startNode, INode endNode, float maxVerticalDistance,
            Action<List<Vector3>> successCallback, Action failureCallback = null)
        {
        ...
        private void Search(INode node)
        {
            ...
            {
                if (!nextNode.Passable || closed.Contains(nextNode) ||
                    Mathf.Abs(node.Position.y - nextNode.Position.y) > _maxVerticalDist)
                    continue;
                //add if not exists
